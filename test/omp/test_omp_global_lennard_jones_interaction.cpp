#define BOOST_TEST_MODULE "test_omp_global_pair_lennard_jones_interaction"
...
BOOST_AUTO_TEST_CASE(omp_GlobalPair_LennardJones_calc_force)
{
    ...
    using omp_parameter_type      = mjolnir::LorentzBerthelotRule<omp_traits_type, potential_type>::parameter_type;
    ...
    using seq_parameter_type      = mjolnir::LorentzBerthelotRule<seq_traits_type, potential_type>::parameter_type;
    ...
    for(int num_thread=1; num_thread<=max_number_of_threads; ++num_thread)
    {
        ...
        {
            omp_parameters[i] = std::make_pair(i, omp_parameter_type{1.0, 1.0});
        }
        std::vector<std::pair<std::size_t, seq_parameter_type>> seq_parameters(N_particle);
        for(std::size_t i=0; i<N_particle; ++i)
        {
            seq_parameters[i] = std::make_pair(i, seq_parameter_type{1.0, 1.0});
        }

        potential_type potential(/*cutoff = */2.5);

        mjolnir::LorentzBerthelotRule<omp_traits_type, potential_type> omp_rule(omp_parameters, {},
        ...
        omp_parameter_list_type omp_parameter_list(mjolnir::make_unique<
            mjolnir::LorentzBerthelotRule<omp_traits_type, potential_type>>(std::move(omp_rule)));

        mjolnir::LorentzBerthelotRule<seq_traits_type, potential_type> seq_rule(seq_parameters, {},
        ...
        seq_parameter_list_type seq_parameter_list(mjolnir::make_unique<
            mjolnir::LorentzBerthelotRule<seq_traits_type, potential_type>>(std::move(seq_rule)));
