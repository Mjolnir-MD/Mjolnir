#define BOOST_TEST_MODULE "test_omp_3spn2_cross_stacking_interaction"
...
BOOST_AUTO_TEST_CASE(ThreeSPN2CrossStackingIntearction_numerical_diff)
{
    ...
    mjolnir::LoggerManager::set_default_logger(
            "test_omp_3spn2_cross_stacking_interaction.log");
    ...
    using potential_type       = mjolnir::ThreeSPN2CrossStackingPotential<real_type>;

    using omp_system_type      = mjolnir::System<omp_traits_type>;
    using omp_interaction_type = mjolnir::ThreeSPN2CrossStackingInteraction<omp_traits_type>;

    using seq_system_type      = mjolnir::System<seq_traits_type>;
    using seq_interaction_type = mjolnir::ThreeSPN2CrossStackingInteraction<seq_traits_type>;

    using omp_parameter_list_type = mjolnir::ThreeSPN2CrossStackingParameterList<omp_traits_type>;
    using seq_parameter_list_type = mjolnir::ThreeSPN2CrossStackingParameterList<seq_traits_type>;
    ...
    for(int num_thread=1; num_thread<=max_number_of_threads; ++num_thread)
    {
        ...
        omp_parameter_list_type omp_parameter_list(
            mjolnir::ThreeSPN2CrossStackingGlobalPotentialParameter<real_type>{}, {
                {1, omp_parameter_type{bases.at(0), 0, 4, invalid}},
                {9, omp_parameter_type{bases.at(1), 8, invalid, 6}},
                {4, omp_parameter_type{bases.at(2), 3, invalid, 1}},
                {6, omp_parameter_type{bases.at(3), 5, 9, invalid}},
            }, {}, ignore_molecule_type("Nothing"), ignore_group_type({})
        );
        seq_parameter_list_type seq_parameter_list(
            mjolnir::ThreeSPN2CrossStackingGlobalPotentialParameter<real_type>{}, {
                {1, seq_parameter_type{bases.at(0), 0, 4, invalid}},
                {9, seq_parameter_type{bases.at(1), 8, invalid, 6}},
                {4, seq_parameter_type{bases.at(2), 3, invalid, 1}},
                {6, seq_parameter_type{bases.at(3), 5, 9, invalid}},
            }, {}, ignore_molecule_type("Nothing"), ignore_group_type({})
        );
        mjolnir::ThreeSPN2BasePairParameterList<omp_traits_type> bp_parameter_list(
        ...
        const auto rbp          =  bp_parameter_list.r0(bp_kind) + 0.2;
        ...
        const auto pi_over_K_BP =  bp_parameter_list.pi_over_K_BP();
