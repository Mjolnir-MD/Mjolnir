#define BOOST_TEST_MODULE "test_read_worm_like_chain_offset_potential"
...
BOOST_AUTO_TEST_CASE_TEMPLATE(read_worm_like_chain_offset_noenv, T, test_types)
{
    mjolnir::LoggerManager::set_default_logger("test_read_worm_like_chain_offset.log");
    ...
    const auto w = mjolnir::read_worm_like_chain_offset_potential<real_type>(param, env);
    BOOST_TEST(w.p()    == real_type(3.9),  tolerance<real_type>());
    BOOST_TEST(w.lc()   == real_type(19.0), tolerance<real_type>());
    BOOST_TEST(w.l0()   == real_type(5.0),  tolerance<real_type>());
}

BOOST_AUTO_TEST_CASE_TEMPLATE(read_worm_like_chain_offset_env, T, test_types)
{
    mjolnir::LoggerManager::set_default_logger("test_read_worm_like_chain_offset.log");
