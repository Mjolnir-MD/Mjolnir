...
BOOST_AUTO_TEST_CASE(LJ_constructable)
{
    std::unique_ptr<mjolnir::GlobalPotentialBase<traits>> lj = 
        mjolnir::make_unique<mjolnir::LennardJonesPotential<traits>>();

    BOOST_CHECK(lj);
}

BOOST_AUTO_TEST_CASE(LJ_derivative)
{
    std::unique_ptr<mjolnir::LennardJonesPotential<traits>> lj = 
        mjolnir::make_unique<mjolnir::LennardJonesPotential<traits>>();

    const traits::real_type sigma   = 3.0;
    const traits::real_type epsilon = 1.0;
    lj->emplace(std::make_pair(sigma, epsilon));
    lj->emplace(std::make_pair(sigma, epsilon));
