...
traj_hist, traj_bin_edges = np.histogram(\
        [float(line.split()[1]) for line in filter(lambda x: x[0:4] == 'CORE',\
               open("data/test_g-BAOABIntegrator_position.xyz").readlines())],\
...
plt.title("g-BAOABIntegrator")
plt.legend()
plt.savefig("data/test_g-BAOABIntegrator.png")
plt.show()
