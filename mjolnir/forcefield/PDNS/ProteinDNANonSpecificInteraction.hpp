        ...
        const system_type& sys) const noexcept
{
    ...
    for(const auto& contact : this->parameters_.contacts())
    {
...
template<bool NeedEnergy, bool NeedVirial>
typename ProteinDNANonSpecificInteraction<traitsT>::real_type
ProteinDNANonSpecificInteraction<traitsT>::calc_force_energy_virial_impl(
        system_type& sys) const noexcept
{
    ...
    for(const auto& contact : this->parameters_.contacts())
    {
        ...
        for(const auto& ptnr : this->partition_.partners(P))
        {
            ...
            const auto& para = ptnr.parameter(); // parameter
            ...
            sys.force(D)       += f_D;
            sys.force(S3)      += f_S3;
            sys.force(contact.PN) += f_PN;
            sys.force(contact.PC) += f_PC;
