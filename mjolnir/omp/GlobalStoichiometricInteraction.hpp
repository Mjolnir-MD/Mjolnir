    ...
    > final : public GlobalInteractionBase<OpenMPSimulatorTraits<realT, boundaryT>>
{
  ...
  public:
          ...
          epsilon_(epsilon), coefa_(coefa), coefb_(coefb)
    {
        ...
        for(std::size_t idx=0; idx<participants_b_num; ++idx)
        {
            pot_sum_b_thread_[idx]      .resize(num_threads_);
            pot_deriv_sum_b_thread_[idx].resize(num_threads_);
        }
    }
...
template<typename realT, template<typename, typename> class boundaryT>
void GlobalStoichiometricInteraction<
    OpenMPSimulatorTraits<realT, boundaryT>>::calc_force(system_type& sys) const noexcept
{
    ...
    for(std::size_t idx=0; idx<participants_b_num; ++idx)
    {
        std::vector<real_type>&       pot_sum_b_thread       = pot_sum_b_thread_[idx];
        std::vector<coordinate_type>& pot_deriv_sum_b_thread = pot_deriv_sum_b_thread_[idx];
...
template<typename realT, template<typename, typename> class boundaryT>
typename GlobalStoichiometricInteraction<OpenMPSimulatorTraits<realT, boundaryT>>::real_type
GlobalStoichiometricInteraction<
...
typename GlobalStoichiometricInteraction<OpenMPSimulatorTraits<realT, boundaryT>>::real_type
GlobalStoichiometricInteraction<
    OpenMPSimulatorTraits<realT, boundaryT> >::calc_force_and_energy(system_type& sys) const noexcept
{
